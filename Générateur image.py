...
import requests
import json
token = "'8032707504:AAHqzqOYBgG1FY6PVe7CcksWq3-HodvsJD4'"
bot = telebot.TeleBot(token)
@bot.message_handler(commands=['start'])
def send_welcome(message):
    bot.reply_to(message, "Hello dear, send the text in English to convert it to an image")
...
def generate_image(message):
	...
	if len(parts) > 1:
		...
		print("There is no link in the text.")
